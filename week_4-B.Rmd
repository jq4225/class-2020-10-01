...
classes <- bind_rows(d_2019, d_2018, d_2017, d_2016, d_2015)
...
big_classes(d_2019, 2019)
```
```{r}
big_classes2 <- function(x) {
  stopifnot(is_tibble(x))
  x %>%
    arrange(desc(u_grad)) %>%
    slice(1:5)
...
plots <- classes2 %>%
  group_by(year) %>%
  group_nest() %>%
  mutate(my_plot = hist(data))

ggplot(data = plots, aes(x = data)) + geom_histogram()
